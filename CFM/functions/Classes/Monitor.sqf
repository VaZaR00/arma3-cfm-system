...
OBJCLASS(Monitor)
	...
	METHOD("Init") { 
		...
		if (_local) then {
			...
			if !(_allowedOperatorsSet isEqualType []) then {_allowedOperatorsSet = [_allowedOperatorsSet]};
			...
			if !(_allowedOperatorsTypesSet isEqualType []) then {_allowedOperatorsTypesSet = [_allowedOperatorsTypesSet]};
