/*
    Function: CFM_fnc_updateMonitorCamera
    Author: Vazar
...
private _opIsDrone = _monitor getVariable ["CFM_currentOperatorIsDrone", false];
private _zoomFov = if (_opIsDrone && {!(MGVAR ["CFM_canChangeZoomOnDrones", false])}) then {
	"op"
} else {
